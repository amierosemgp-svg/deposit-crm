...
import { isBotOnline } from "@/lib/format";
...
export function CompanyInfoPanel({ month }: { month: string | "all" }) {
  ...
  const scope = useMemo(() => {
    // Same resolution as the dashboard: connectivity comes from the agent
    // process driving the bank/game, not from the account rows themselves.
    const bankOnline = (bankName: string) =>
      isBotOnline(botForName(botHealth, bankName)?.last_heartbeat_at);
    const gameOnline = (gameName: string) =>
  ...
  }, [bankAccounts, boAccounts, deposits, withdrawals, players, month, selectedCompanyId, companyInScope, botHealth]);
