...
# ── Names of the tree this import creates ────────────────────────────────────
MAIN_COMPANY = "RajaClub Group"
LEADER = "RajaClub Leader"
COMPANY = "RajaClub"
CS_DESK = "RajaClub CS Desk"
EMAIL_DOMAIN = "rajaclub.local"

USERS = [
    # username,    role,             entity, full name
    ("rc_admin",   "super_admin",    "main",    "RajaClub Super Admin"),
    ("rc_leader",  "company_leader", "leader",  "RajaClub Leader"),
    ("rc_cs",      "cs_agent",       "cs",      "RajaClub CS Agent"),
    ("rc_viewer",  "viewer",         "main",    "RajaClub Viewer"),
]
...
SHEETS = {
    "Deposit":     dict(header=18, width=14),
    "Withdrawal":  dict(header=17, width=9),
    "Free Credit": dict(header=11, width=9),
}
...
GAME_ALIASES = {
    "MG888": "Mega888",
    "Mega": "Mega888",          # the kiosk list's spelling of the same product
    "JK123": "Joker",
    "Joker123": "Joker",        # likewise
    "Scr918Kiss": "918Kiss",
}
...
def parse_dashboard(wb):
    """
    The summary block at the top of the Deposit sheet: per-bank opening balance
    with the month's movement, per-game closing credit, and the per-game
    deposit/bonus/withdrawal/free-credit matrix. This is what every derived
    total is checked against.
    """
    ws = wb["Deposit"]
    cell = lambda r, c: ws.cell(row=r, column=c + 1).value

    banks = {}
    for r in range(3, 10):
        for base in (21, 26):           # "Bank 1" block, then "Bank 2"
            code = S(cell(r, base))
            if not code or code in ("Opening", "Deposit", "Withdrawal", "4D"):
                continue
            banks[code] = dict(
                opening=D(cell(r, base + 1)),
                deposit=D(cell(r, base + 2)),
                # The Bank 2 block carries an extra column before withdrawal.
                withdrawal=D(cell(r, base + 3 if base == 21 else base + 4)),
            )

    # Closing credit per kiosk, listed down the GAME and GAME 2 columns. The
    # second block names its entries "<game> 2" — a second account on the same
    # product, not a product of its own, so the suffix is the account's label.
    kiosks = []
    for r in range(2, 17):
        for base, group in ((12, "1"), (15, "2")):
            name = S(cell(r, base))
            if name and not re.fullmatch(r"[\d.]+", name):
                kiosks.append((re.sub(r"\s+2$", "", name), name, group,
                               D(cell(r, base + 1))))

    # The movement matrix. Row 2 heads the columns; rows 3-9 label themselves in
    # the column just left of the block.
    matrix = {}
    for base, span in ((33, 15), (49, 6)):
        heads = [S(cell(2, base + i)) for i in range(span)]
        for r in range(3, 10):
            label = S(cell(r, base - 1))
            if not label:
                continue
            for i, head in enumerate(heads):
                if head:
                    matrix.setdefault(label, {})[head] = D(cell(r, base + i))

    return dict(banks=banks, kiosks=kiosks, matrix=matrix)
...
def parse(path):
    ...
    for n, row in Sheet(wb, "Deposit").rows:
        remark, contact, date, time, code, login, product, pct, bank, amount, bonus, kwn, kwn_amt, _ = row
    ...
    for n, row in Sheet(wb, "Free Credit").rows:
        date, time, code, login, product, amount, _, remark, _ = row
        ...
        free_credits.append(dict(row=n, at=ts, code=code, login=login, product=product,
                                 amount=money(D(amount)), remark=S(remark)))
    ...
    out.update(members=members, deposits=deposits, referrals=referrals,
               ...
               cash_outs=cash_outs, free_credits=free_credits)
...
def reconcile(data):
    ...
    (label, expected, got) — empty means every total agrees to the cent.
    """
    dash = data["dashboard"]
    checks = []

    # Per bank: what the transaction rows say moved, vs the dashboard's figure,
    # and whether opening + in − out lands on the balance the sheet reports.
    dep_by_bank = collections.Counter()
    for r in data["deposits"]:
        dep_by_bank[r["bank"]] += r["amount"]
    wd_by_bank = collections.Counter()
    for r in data["withdrawals"] + data["cash_outs"]:
        wd_by_bank[r["bank"]] += r["amount"]

    for code, fig in sorted(dash["banks"].items()):
        if fig["deposit"] == 0 and fig["withdrawal"] == 0 and code not in dep_by_bank \
                and code not in wd_by_bank:
            continue
        checks.append((f"bank {code} deposits", fig["deposit"], dep_by_bank.get(code, Decimal(0))))
        checks.append((f"bank {code} withdrawals", fig["withdrawal"], wd_by_bank.get(code, Decimal(0))))

    # Per game: deposits, bonus, withdrawals and free credit, against the
    # dashboard's movement matrix.
    m = dash["matrix"]
    # The dashboard's per-game "Deposit" is a sum of the Amount column keyed on
    # Product, so it counts both legs of every ID TO ID move alongside the real
    # bank credits. Those legs cancel across games but not within one, so the
    # check has to include them even though they are imported as transfers.
    dep_by_game = collections.Counter()
    bonus_by_game = collections.Counter()
    for r in data["deposits"] + data["id_to_id"]:
        if r["product"]:
            dep_by_game[r["product"]] += r["amount"]
    for r in data["deposits"] + data["referrals"] + data["bonus_only"]:
        if r["product"]:
            bonus_by_game[r["product"]] += r["bonus"]
    wd_by_game = collections.Counter()
    for r in data["withdrawals"]:
        wd_by_game[r["product"]] += r["amount"]
    fc_by_game = collections.Counter()
    for r in data["free_credits"]:
        fc_by_game[r["product"]] += r["amount"]

    for label, got in (("Deposit", dep_by_game), ("Bonus", bonus_by_game),
                       ("Withdrawal", wd_by_game), ("Free Credit", fc_by_game)):
        expected = m.get(label, {})
        for game in sorted(set(expected) | set(got)):
            e, g = expected.get(game, Decimal(0)), got.get(game, Decimal(0))
            if e or g:
                checks.append((f"game {game} {label.lower()}", e, g))

    # And the four grand totals.
    checks.append(("TOTAL deposits", sum(m.get("Deposit", {}).values(), Decimal(0)),
                   sum(dep_by_game.values(), Decimal(0))))
    checks.append(("TOTAL bonus", sum(m.get("Bonus", {}).values(), Decimal(0)),
                   sum(bonus_by_game.values(), Decimal(0))))
    checks.append(("TOTAL withdrawals", sum(m.get("Withdrawal", {}).values(), Decimal(0)),
                   sum(wd_by_game.values(), Decimal(0))))
    checks.append(("TOTAL free credit", sum(m.get("Free Credit", {}).values(), Decimal(0)),
                   sum(fc_by_game.values(), Decimal(0))))

    return [(l, e, g) for l, e, g in checks if money(e) != money(g)]
...
def build_sql(data, password_hash, games_catalogue, banks_catalogue, replace):
    ...
    if replace:
        add(f"""
-- Undo a previous run of this importer: everything hangs off the entity tree.
CREATE TEMP TABLE old_ent ON COMMIT DROP AS
  WITH RECURSIVE t AS (
    SELECT entity_id FROM entities WHERE name = {lit(MAIN_COMPANY)} AND entity_type = 'main_company'
    UNION ALL SELECT e.entity_id FROM entities e JOIN t ON e.parent_entity_id = t.entity_id)
  SELECT entity_id FROM t;
CREATE TEMP TABLE old_pl ON COMMIT DROP AS
  SELECT player_id FROM players WHERE company_entity_id IN (SELECT entity_id FROM old_ent);
DELETE FROM transactions       WHERE player_id IN (SELECT player_id FROM old_pl)
                                  OR entity_id IN (SELECT entity_id FROM old_ent);
...
DELETE FROM deposits           WHERE company_entity_id IN (SELECT entity_id FROM old_ent);
DELETE FROM bank_cash_outs     WHERE entity_id IN (SELECT entity_id FROM old_ent);
DELETE FROM member_game_accounts WHERE member_id IN (SELECT player_id FROM old_pl);
DELETE FROM member_bank_accounts WHERE member_id IN (SELECT player_id FROM old_pl);
UPDATE players SET upline_player_id = NULL WHERE player_id IN (SELECT player_id FROM old_pl);
-- The people these members point at have to be noted before the members go,
-- and dropped after: players.person_id is what makes them findable.
...
DELETE FROM provider_bo_accounts WHERE company_entity_id IN (SELECT entity_id FROM old_ent);
DELETE FROM bank_accounts      WHERE entity_id IN (SELECT entity_id FROM old_ent);
DELETE FROM bonus_plans        WHERE company_entity_id IN (SELECT entity_id FROM old_ent);
-- Anything the imported logins have done since — signing in leaves a device,
-- a challenge and an audit trail, and all three pin the user row in place.
CREATE TEMP TABLE old_user ON COMMIT DROP AS
  SELECT user_id FROM users WHERE entity_id IN (SELECT entity_id FROM old_ent);
DELETE FROM activity_log   WHERE actor_user_id IN (SELECT user_id FROM old_user)
                              OR company_entity_id IN (SELECT entity_id FROM old_ent);
DELETE FROM auth_challenges WHERE user_id IN (SELECT user_id FROM old_user);
DELETE FROM user_devices   WHERE user_id IN (SELECT user_id FROM old_user)
                              OR approved_by_user_id IN (SELECT user_id FROM old_user);
DELETE FROM users              WHERE user_id IN (SELECT user_id FROM old_user);
DELETE FROM entities           WHERE entity_id IN (SELECT entity_id FROM old_ent);
""")
    else:
        add(f"""
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM entities WHERE name = {lit(MAIN_COMPANY)} AND entity_type = 'main_company') THEN
    RAISE EXCEPTION '{MAIN_COMPANY} already exists — re-run with --replace to overwrite it';
  END IF;
END $$;""")
...
INSERT INTO entities (entity_type, name) VALUES ('main_company', {lit(MAIN_COMPANY)});
INSERT INTO ctx VALUES ('main', currval('entities_entity_id_seq'));
INSERT INTO entities (parent_entity_id, entity_type, name)
  SELECT v, 'leader', {lit(LEADER)} FROM ctx WHERE k = 'main';
INSERT INTO ctx VALUES ('leader', currval('entities_entity_id_seq'));
INSERT INTO entities (parent_entity_id, entity_type, name)
  SELECT v, 'company', {lit(COMPANY)} FROM ctx WHERE k = 'leader';
INSERT INTO ctx VALUES ('company', currval('entities_entity_id_seq'));
INSERT INTO entities (parent_entity_id, entity_type, name)
  SELECT v, 'cs', {lit(CS_DESK)} FROM ctx WHERE k = 'company';
INSERT INTO ctx VALUES ('cs', currval('entities_entity_id_seq'));""")

    # ── 2. users ─────────────────────────────────────────────────────────────
    for username, role, where, full_name in USERS:
        add(f"""
INSERT INTO users (username, email, full_name, password_hash, role, entity_id)
  SELECT {lit(username)}, {lit(f'{username}@{EMAIL_DOMAIN}')}, {lit(full_name)},
         {lit(password_hash)}, {lit(role)}::user_role, v FROM ctx WHERE k = {lit(where)};
INSERT INTO ctx VALUES ({lit('user_' + username)}, currval('users_user_id_seq'));""")
    add("CREATE TEMP VIEW cs_user AS SELECT v FROM ctx WHERE k = 'user_rc_cs';")
  ...
  WHERE key = 'banks';""")
    ...
    for code in banks:
        ...
        bank_rows.append((
            ...
            "deposit" if code in dep_banks else "withdrawal",
            money(opening + deposit - withdrawal),
            money(opening),
        ))
    add("""
CREATE TEMP TABLE imp_bank (code text PRIMARY KEY, bank_name text, role text,
                            closing numeric, opening numeric, account_id int) ON COMMIT DROP;""")
    add(copy_block("imp_bank", ["code", "bank_name", "role", "closing", "opening"], bank_rows))
    add(f"""
...
INSERT INTO bank_accounts (account_id, entity_id, role, bank_name, account_number,
  ...
  SELECT b.account_id, c.v, b.role::bank_account_role, b.bank_name,
         'RC-' || b.code, {lit(COMPANY)}, b.code, b.closing, 'active'
...
def report(data):
    ...
    print("\nbank ledger (from the sheet's own dashboard):")
    ...
    for r in d["withdrawals"] + d["cash_outs"]:
        wd[r["bank"]] += r["amount"]
    print(f"  {'bank':12}{'opening':>13}{'in':>14}{'out':>14}{'closing':>13}")
    for code, fig in sorted(d["dashboard"]["banks"].items()):
        if not (fig["deposit"] or fig["withdrawal"] or fig["opening"]):
            continue
        closing = fig["opening"] + fig["deposit"] - fig["withdrawal"]
        print(f"  {code:12}{fig['opening']:>13}{fig['deposit']:>14}"
              f"{fig['withdrawal']:>14}{money(closing):>13}")
...
WITH RECURSIVE tree AS (
  SELECT entity_id FROM entities WHERE name = %(main)s AND entity_type = 'main_company'
  UNION ALL SELECT e.entity_id FROM entities e JOIN tree t ON e.parent_entity_id = t.entity_id),
co AS (SELECT entity_id FROM entities WHERE parent_entity_id IN (SELECT entity_id FROM tree)
...
def verify(dsn, data):
    ...
    rows = psql(dsn, VERIFY_SQL.replace("%(main)s", lit(MAIN_COMPANY)), quiet=True)
    ...
    expect = {
        ...
        "bank closing total": money(sum(
            f["opening"] + f["deposit"] - f["withdrawal"]
            for c, f in d["dashboard"]["banks"].items()
            if f["deposit"] or f["withdrawal"] or f["opening"])),
...
def main():
    ...
    sql = build_sql(data, bcrypt_hash(args.password, here),
                    catalogue + new_games if new_games else None,
                    banks + new_banks if new_banks else None,
                    args.replace)
    path = args.sql_out or os.path.join(tempfile.mkdtemp(), "import-rajaclub.sql")
    ...
    print(f"\nlogins: {', '.join(u for u, _, _, _ in USERS)} — password {args.password!r}")
